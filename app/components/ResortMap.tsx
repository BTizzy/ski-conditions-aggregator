...
const ResortMap: React.FC<ResortMapProps> = ({
  ...
  // Load radar frames - try SYNTHETIC first, fallback to RainViewer
  useEffect(() => {
    if (!mapReady) return;

    const loadFrames = async () => {
      try {
        // Try synthetic first (your resort data)
        console.log('[Frames] Trying synthetic frames...');
        const synthRes = await fetch('/api/radar/synthetic-frames', { signal: AbortSignal.timeout(3000) });
        ...
        console.log('[Frames] Synthetic failed/empty, trying RainViewer...');
        const rainRes = await fetch('/api/radar/frames');
        if (!rainRes.ok) throw new Error('RainViewer failed');

        const data = await rainRes.json();
        const layers = data?.radar?.layers || [];
        console.log('[Frames] Using RainViewer:', layers.length, 'frames');
        ...
        const sourceLabel = data?.radar?.source === 'rainviewer-48h' ? '48h RainViewer' : '1h Mesonet';
  ...
  return (
    <div className="relative w-full h-screen bg-gray-100 flex flex-col">
      ...
      <div className="absolute top-4 left-4 z-[99999] bg-white/95 rounded-lg p-4 shadow-lg max-w-xs">
        ...
        <div className="text-xs text-blue-600 font-semibold mb-2">{radarSource === 'synthetic' ? '🔬 Synthetic (Resort Data)' : '📡 RainViewer'}</div>
